...
public class RespuestaServiceListarPorUsuario {
    ...
    @Test
    void deberiaListarRespuestasPorUsuario() {
        ...
        when(respuestaRepository.buscarTodosPorUsuario(idUsuario))
                .thenReturn(List.of(r1, r2));

        when(respuestaDTOMapper.toResponse(r1)).thenReturn(resp1);
        when(respuestaDTOMapper.toResponse(r2)).thenReturn(resp2);

        List<RespuestaResponse> resultado = respuestaService.listarPorUsuario(idUsuario);

        assertNotNull(resultado);
        assertEquals(2, resultado.size());

        verify(respuestaRepository).buscarTodosPorUsuario(idUsuario);
    }

    @Test
    void deberiaRetornarListaVaciaSiUsuarioNoTieneRespuestas() {

        when(respuestaRepository.buscarTodosPorUsuario(any()))
                .thenReturn(List.of());

        List<RespuestaResponse> resultado = respuestaService.listarPorUsuario(1L);
